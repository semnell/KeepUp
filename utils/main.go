...
func RegisterWorkers(conf Config) {
	...
	for _, job := range conf.Jobs {
		...
		if err != nil {
			logger.Error("Error marshalling job to JSON: ", zap.Error(err))
			panic(err)
		}
...
func postJob(interval int, obj string) {
	...
	logger := SetupLogger()
	if err != nil {
		logger.Error("Error opening Faktory client: ", zap.Error(err))
	}
	...
	for range time.Tick(time.Second * time.Duration(interval)) {
		...
		err = client.Push(job)
		if err != nil {
			logger.Error("Error pushing job to Faktory: ", zap.Error(err))
			panic(err)
		}
	}
}
